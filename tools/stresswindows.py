...
if IS_WINDOWS:
    ...
    def _call_nt(func, proc_handle):
        handle = ctypes.c_void_p(proc_handle._handle)
        status = func(handle)
        if status != 0:
            raise OSError("{} failed with status {}".format(func.__name__, status))


    def suspend_process(proc_handle):
        _call_nt(_NtSuspendProcess, proc_handle)


    def resume_process(proc_handle):
        _call_nt(_NtResumeProcess, proc_handle)


else:

    def suspend_process(proc_handle):
        proc_handle.send_signal(signal.SIGSTOP)


    def resume_process(proc_handle):
        proc_handle.send_signal(signal.SIGCONT)
...
class ProcessInfo:
    ...
    def stateToSignal(state):
        if IS_WINDOWS:
            raise RuntimeError("Signals are not supported on Windows")
    ...
    def applyState(handle, state):
        if IS_WINDOWS:
            ...
            if state == ProcessState.TERMINATED:
                # On POSIX terminate() uses SIGTERM by default
                handle.terminate()
            else:
                handle.send_signal(ProcessInfo.stateToSignal(state))
        ...
        while successfulAttempts < self.attempts:
            ...
            with info.lock:
                if ProcessInfo.validStateTransition(info.state, op):
                    ...
                    try:
                        ProcessInfo.applyState(info.handle, op)
                    except OSError as exc:
                        print(
                            "Failed to send {} to process {}: {}".format(
                                ProcessInfo.stateToSignalStr(op), proc, exc
                            )
                        )
                        continue
            ...
            with info.lock:
                if info.state != ProcessState.TERMINATED:
                    if info.state == ProcessState.STOPPED:
                        try:
                            ProcessInfo.applyState(
                                info.handle, ProcessState.RUNNING
                            )
                        except OSError as exc:
                            print(
                                "Failed to resume process {} during termination: {}".format(
                                    pid, exc
                                )
                            )
                    ...
                    try:
                        ProcessInfo.applyState(
                            info.handle, ProcessState.TERMINATED
                        )
                    except OSError as exc:
                        print(
                            "Failed to terminate process {}: {}".format(pid, exc)
                        )
                ...
                if info.state != ProcessState.TERMINATED:
                    if info.state == ProcessState.STOPPED:
                        try:
                            ...
                            print(
                                "Failed to resume process {}: {}".format(pid, exc)
                            )
...
def startProcesses(processes, runscript, hostsFilePath, configFilePaths, outputDir):
    ...
    runscriptName = os.path.basename(runscriptPath).lower()
    if runscriptName not in ("run.sh", "run.ps1"):
        raise Exception(
            "`{}` is not a recognized run script (expected run.sh or run.ps1)".format(
                runscriptPath
            )
        )
